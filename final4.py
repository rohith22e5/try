...
class DocumentOrganizer(QMainWindow):
    ...
    def process_individual_file(self, file_path):
        if self.is_folder_categorized(self.input_folder):
            ...
            output_folder_path = os.path.join(self.output_folder, folder_to_copy)
    ...
    def generate_category_names(self, categorized_files, vectorizer):
        category_texts = [" ".join([self.extract_text_from_file(file) for file in files]) for files in categorized_files.values()]
        lda = LDA(n_components=len(category_texts), random_state=42)
        X = vectorizer.fit_transform(category_texts)
        lda.fit(X)
        terms = vectorizer.get_feature_names_out()
        category_names = {}
        for idx, topic in enumerate(lda.components_):
            topic_name = " ".join([terms[i] for i in topic.argsort()[:-6:-1]])
            category_names[f"Category_{idx + 1}"] = topic_name
    ...
    def organize_files_into_folders(self, categorized_files, output_dir, progress_bar):
        ...
        # Generate descriptive category names
        vectorizer = CountVectorizer(stop_words='english')
        category_names = self.generate_category_names(categorized_files, vectorizer)
        ...
        for category, files in categorized_files.items():
            ...
            for file in files:
                ...
                processed_files += 1
                progress_bar.setValue(int((processed_files / total_files) * 100))
